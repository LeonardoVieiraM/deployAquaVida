<script lang="ts">
	...
	let local = '';
	let descricao = '';
	let selectedClienteId = data.cliente.id.toString();
	let selectedRepresentanteId = data.representante[0].id.toString();
	...
	let options: CalendarOptions = {
		...
		dateClick: (info) => {
			...
			meetingName = '';
			meetingTime = '09:00';
			local = String(data.cliente.endereco);
			descricao = '';
			isEdit = false;
			showModal = true;
		},
	...
	<div class="modal-overlay" on:click={closeModal}>
		<div class="modal-content" on:click|stopPropagation>
			...
			<form method="POST" action={isEdit ? '?/update' : '?/create'}>
				...
				<input type="hidden" name="representante_id" value={selectedRepresentanteId} />
