		...
		<div class="modal-content" on:click|stopPropagation>
			...
			<form method="POST" action={isEdit ? '?/update' : '?/create'}>
				...
				<div class="form-group">
					...
					<input id="local" type="text" name="local" bind:value={local} required />
				</div>
				...
				<div class="form-group">
					...
					<select name="cliente_id" bind:value={selectedClienteId} required>
				...
				<div class="buttons">
	<button type="submit" class="add-btn">{isEdit ? 'Atualizar Evento' : 'Adicionar Evento'}</button>
	<button type="button" class="cancel-btn" on:click={closeModal}>Cancelar</button>

	{#if isEdit}
		<form method="POST" action="?/delete">
			<input type="hidden" name="id" value={eventId} />
			<button type="submit" class="neutral-btn" style="background-color: gray; color: white;">Excluir Evento</button>
		</form>
	{/if}
</div>
